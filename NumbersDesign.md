...
> 2026-03-20 업데이트:
> Gemini CLI가 활동 HTML을 직접 생성하는 자유 카드 방식도 이 문서를 따라야 합니다. 즉, 템플릿 기반이든 자유 생성이든 결과 HTML은 아래 규칙을 반드시 만족해야 합니다.
...
> [!IMPORTANT]
> **자유 생성 HTML도 예외가 아닙니다.** Gemini가 활동 카드를 직접 만들 때도, 검정 테두리 작성 영역과 연한 배경 컨테이너 구조를 반드시 유지해야 합니다.
...
> [!IMPORTANT]
> **입력 영역의 크기가 충분해야 합니다.** Numbers에서 학생들은 타이핑보다 **Apple Pencil 필기**를 많이 사용하므로, 세로 길이를 이전보다 더 넉넉하게 잡아야 합니다. 일반 작성 영역은 최소 `220px`, 메인 작성 영역은 최소 `420px`, 핵심 서술 영역은 가능하면 `520px+`를 확보합니다.

> [!IMPORTANT]
> **자유 활동 카드의 세로 예산을 늘립니다.** 카드 하나가 지나치게 납작하면 학생 필기가 어려워집니다. Gemini가 생성하는 카드형 HTML은 가능하면 `1600px` 폭 기준에서 전체 카드 높이가 `1200px~1800px` 범위가 되도록 설계합니다. 주 활동이 서술형이면 더 길어도 됩니다.

## 2.7 🤖 자유 생성 HTML 원칙 (Gemini Freeform Card Rules)

Gemini CLI가 활동 HTML을 직접 생성할 때는 아래 원칙을 추가로 지켜야 합니다.

### 필수 원칙
- 전체 문서는 **완전한 HTML 문서**여야 한다.
- `body` 기준 기본 폭은 `1600px`를 사용한다.
- 활동 카드는 하나의 큰 루트 컨테이너로 감싼다.
- 학생이 실제로 필기하는 영역은 흰색 배경 + `2px solid #333` 테두리를 유지한다.
- 작은 장식 요소보다 **쓰기 공간 확보**를 우선한다.
- 한 카드 안에 정보와 입력 영역이 지나치게 많아지면 안 된다.

### 권장 레이아웃
- 상단: 배지 + 활동 제목 + 짧은 안내문
- 중단: 자료/개념/질문 블록
- 하단: 큰 작성 영역 또는 다중 작성 영역
- 서술형 활동은 하단의 메인 작성 영역을 가장 크게 둔다.
...
- 과도하게 촘촘한 4단 이상 그리드
- 지나치게 작은 폰트
- 필기 공간보다 설명문이 더 많은 구성
- 스크린샷 시 잘리는 긴 가로 배너형 디자인
...
배움 노트는 단순 요약 칸이 아니라, **수업시간에 들은 설명과 교과서 핵심 내용을 함께 정리하는 칸**으로 사용합니다.
...
설계 원칙:
- 메인 노트 영역은 다른 카드보다 세로 길이를 더 크게 준다.
- 학생이 필기를 길게 남길 수 있도록 줄 간격 노트 또는 큰 자유 필기 박스를 사용한다.
- 보조 요약/질문 칸보다 메인 노트 칸이 가장 커야 한다.
...
<section class="activity-container" style="--section-bg: #FFF8E7;">
  ...
  <div class="learning-note">
    <div class="note-header">
      <h2>배움 노트</h2>
      <p class="note-subtitle">수업시간에 들은 내용 정리하기</p>
